...
fn main() {
    // Without this, every tracing::info!/warn! call below (including inside
    // refresh_collation_versions) is a silent no-op — no global subscriber
    // means nothing is ever written anywhere. Every other binary in this
    // crate already does this; this one didn't.
    let _guard = init_logging("on-role-change");
