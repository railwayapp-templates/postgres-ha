...
if [ "$PGWAL_BYTES" -ge "$PGWAL_THRESHOLD_BYTES" ]; then
  ...
  # Signal to backup_watcher that a gap was just created. The watcher takes
  # a fresh full backup once archiving recovers, sealing the gap forward
  # (the dropped segment itself is unrestorable, as before).
